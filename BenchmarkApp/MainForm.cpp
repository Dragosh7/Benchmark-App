...
void BenchmarkApp::MainForm::info()
{
    getCPUInfo();
    ...
    if (GlobalMemoryStatusEx(&memoryStatus)) {
        ...
        std::cout << "Random Access Memory: " << (totalMemory / 1024) / 1024 / 1000 << "GB" << std::endl;
    }
    else {
...
void network()
{
    ...
    std::cout << "Server response: " << response.str() << std::endl;

    // Print the transfer time
    double transferTime = duration.count() / 1e6;  // Convert microseconds to seconds
    std::cout << "Total transfer time to www.google.com: " << transferTime << " seconds\n";
...
void BenchmarkApp::MainForm::networkMeasurement()
{
    // Initialize WinSock
    WSADATA wsaData;
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0)
    ...
    std::cout << "Total transfer network time " << ": " << result/3 << " seconds\n";
...
void writeToFile(const std::string& filename) {
    ...
    const std::string contentToWrite =
        "This is a tough file write and read test!\n"
        "It involves writing and reading a significant amount of data to simulate a challenging scenario.";
...
void BenchmarkApp::MainForm::dataMeasurement()
{
...
void calculateFibInThread(int n) {
    ...
    // Measure the time taken by each thread
    auto stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);

    std::cout  << "Thread nr:" << n << ", Time: " << duration.count() / 1e6 << " seconds" << std::endl;
}
void computeFibonacciMultithreaded() {
...
void BenchmarkApp::MainForm::cpuMeasurement(){
    computeFibonacciMultithreaded();
}
